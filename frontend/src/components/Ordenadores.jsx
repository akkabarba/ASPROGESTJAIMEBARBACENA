  ...
  function Ordenadores() {
    ...
    const handleEliminar = async () => {
      ...
      try {
        const token = await refreshTokenIfNeeded();
        const res = await fetch(`${API_BASE}/ordenadores/${modalEliminar.id}/`, {
          method: 'DELETE',
          headers: { Authorization: `Bearer ${token}` }
        });
        if (res.ok) {
          ...
          <>
            <div className="card shadow p-4 mb-3">
              ...
              <div className="d-flex justify-content-center gap-3 mt-3">
                ...
                <button className="btn btn-secondary" onClick={() => setSeleccionado(null)}>
                  Volver
                </button>
              </div>
            </div>
          ...
          <div className="modal d-block bg-dark bg-opacity-50">
            <div className="modal-dialog">
              <div className="modal-content p-4">
                <h5>Eliminar ordenador SN: {modalEliminar.numero_serie}</h5>
                <p className="text-danger mb-2">
                  Escribe el número de serie exacto para confirmar:
                </p>
                <input
